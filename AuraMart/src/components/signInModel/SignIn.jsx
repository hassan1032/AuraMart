...
const SignIn = ({ onClose, onSwitchToRegister }) => {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      if (res.status === 200 || res?.data?.success || res?.data?.status) {
        ...
        setTimeout(() => { onClose(); navigate('/register-otp'); }, 1500);
      } else {
        setError(res?.data?.message || 'Failed to send OTP. Please try again.');
      }
    } catch (err) {
      setError(err.response?.data?.message || (err.code === 'ERR_NETWORK' ? 'Network error. Please check your connection.' : 'Something went wrong. Please try again.'));
    } finally {
      setLoading(false);
    }
  };
      ...
      >
        ...
        <div className="bg-[#E63946] px-6 py-5 flex items-start justify-between">
          <div>
            ...
            <p className="text-blue-200 text-sm mt-0.5">Get access to your Orders, Wishlist and Recommendations</p>
          </div>
        ...
        <div className="p-6">
          <form onSubmit={handleSubmit} noValidate className="space-y-4">
            <div>
              <label className="block text-sm font-medium text-gray-700 mb-1.5">Email Address or Phone</label>
              <div className="relative">
                ...
                <input
                  ...
                  className={`w-full h-10 pl-9 pr-3 border rounded text-sm focus:outline-none focus:ring-2 focus:ring-[#E63946] ${
                    error ? 'border-red-400 focus:ring-red-200' : 'border-[#EAEAEA]'
                  }`}
                />
              </div>
              {error && <p className="text-red-500 text-xs mt-1.5">{error}</p>}
              {success && <p className="text-[#7CB342] text-xs mt-1.5">✓ {success}</p>}
            </div>
          ...
          <p className="text-center text-sm text-gray-600 mt-5">
            New to Mackfarlane?{' '}
            <button onClick={onSwitchToRegister} className="text-[#E63946] font-semibold hover:underline">
              Create Account
            </button>
          </p>
        </div>
